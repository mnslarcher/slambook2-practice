...
struct CURVE_FITTING_COST {
  ...
  template <typename T>
  bool operator()(const T *const abc,  // the estimated variables, 3D vector
                  T *residual) const {
    // y - exp(ax^2 + bx + c)
    residual[0] =
...
int main(int argc, char **argv) {
  ...
  for (int i = 0; i < N; i++) {
    problem.AddResidualBlock(  // add i-th residual into the problem
                               // use auto-diff, template params: residual type,
                               // output dimension, input dimension should be as
                               // the struct written before
  ...
  ceres::Solver::Options
      options;  // actually there're losts of params can be adjusted
